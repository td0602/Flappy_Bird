...
def create_pipe(): # Hàm để tạo ra ống
    ...
    top_pipe = pipe_surface.get_rect(midtop = (500, random_pipe_pos-650)) # Ống trên; -650 là khoảng cachs ông trên ống dưới
    return bottom_pipe, top_pipe #
def move_pipe(pipes): # Hàm di chuyển những cái ống
    for pipe in pipes: # Vòng lặp những cái ống
        pipe.centerx -= 5 # Lấy những ống vưà được tạo ra và di chuyển sang bên trái
...
bird_rect = bird.get_rect(center = (100,384)) # Tạo hình chữ nhật xung quanh con chim, đặt ở vị trí chính giữa của sỏ game (100, 384) 
...
while True: # Vòng lặp cho sự kiện
    for event in pygame.event.get(): # Lấy tấ cả sự kiện cảu pygame diễn ra
        ...
        if event.type == pygame.KEYDOWN: # Thêm event tạo hiệu ứng trọng lực, khi có 1 phím nào trên máy tính dc ấn xuống:
            if event.key == pygame.K_SPACE and game_active: # Nếu bấm phím SPACE và game vẫn đang hđ
                ...
                bird_movement =- 11 # giảm tọa độ trục y của con chim, cũng là độ dao đôgnj con chim nhảy lên nhảy xuống
